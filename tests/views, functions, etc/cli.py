...
def run(module, sql_file, out_file, kwargs):
    ...
    for line in sql_file:
        ...
        if "create_collate" in conn_name:
            action, sql_name, python_name, actual_conn_name = conn_name.split()
            conn = conns[actual_conn_name]
            conn.create_collation(sql_name, locals()[python_name])
            continue
